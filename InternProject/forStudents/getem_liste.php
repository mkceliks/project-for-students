...
<head>
  ...
  <header><h1 class="text">-----Ders Notları(Görkem Etem Irmak)-----</h1></header>
  <meta charset="UTF-8">
</head>
<body>
           <div id="dialog" title="Öğrenci Detayı">
<p><?php    
      $sorgu = "SELECT * FROM info_datamng WHERE info_id = '7'";
...
<button style="margin-right: 10px;" id="opener">Öğrenci Detayı</button><br><br>
  <table>
  <?php 
    $sorgu = "SELECT * FROM datamng WHERE ogrenci_id ='7'";
    $sorguSonucu = mysqli_query($conn, $sorgu);
    if($sorguSonucu) {
  ...
  ?>
  <?php 
    $sorgu = "SELECT * FROM datastr WHERE ogrenci_id ='7'";
    $sorguSonucu = mysqli_query($conn, $sorgu);
    if($sorguSonucu) {
      echo "<table id='listetablosu' >
      <tr>
  ...
  ?>
  <?php 
    $sorgu = "SELECT * FROM digidesign WHERE ogrenci_id ='7'";
    $sorguSonucu = mysqli_query($conn, $sorgu);
    if($sorguSonucu) {
      echo "<table id='listetablosu' >
      <tr>
  ...
  ?>
  <?php 
    $sorgu = "SELECT * FROM electronics WHERE ogrenci_id ='7'";
    $sorguSonucu = mysqli_query($conn, $sorgu);
    if($sorguSonucu) {
      echo "<table id='listetablosu' >
      <tr>
  ...
  echo "<tr><td> Electronics </td><td>" . $kayit ['ogrenci_vize'] . "</td><td>" . $kayit ['ogrenci_final'] . "</td><td>" . $kayit ['ogrenci_quiz'] . "</td><td>" . $kayit ['ogrenci_devam'] ."</tr>";
    ...
    echo "</table><br>";
  ?><?php 
    $sorgu = "SELECT * FROM toc WHERE ogrenci_id ='7'";
    $sorguSonucu = mysqli_query($conn, $sorgu);
    if($sorguSonucu) {
      echo "<table id='listetablosu' >
      <tr>
...
<button class="mainpage" type="submit" name="mainpage" onclick="location.href='getem_index.php'">Ana Sayfa</button></div>
