...
<p><?php if(isset($_GET['id'])){
            $id=$_GET['id'];
            $sorgu = "SELECT * FROM info_digilab WHERE info_id = '$id'";
...
<button style="margin-right: 10px;" id="opener">Öğrenci Detayı</button>
	<div class="text">
   	...
   	<br>
        ...
        if(isset($_GET['id'])){
            ...
            $sql = "SELECT * FROM digilab WHERE ogrenci_id= '$info_id' ";
	...
	<?php 
		$sorgu = "SELECT * FROM digilab WHERE ogrenci_id ='$info_id'";
		$sorguSonucu = mysqli_query($conn, $sorgu);
		if($sorguSonucu) {
        ...
        if(isset($_GET['id'])){
            ...
            if(isset($_POST['submit'])){
                ...
                if (isset($_POST['success']) && $_POST['success'] == 'Success'){ 
                    ...
                    $sql = "UPDATE digilab SET ogrenci_vize='$name', ogrenci_final='$l_name'
                    ...
                    header("Location:../errors/digilab_thankyou.php?id=".$id);
...
<a style="margin-right: 58px;" href="digilab_index.php">Ana Sayfa</a>
</div>
</body>
</html>